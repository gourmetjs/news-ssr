...
function getSaved({page}) {
  return Promise.resolve().then(() => {
    ...
    const pages = Math.floor(saved.length / NEWS_PAGE_SIZE);
  
    if (page < 1 || (page - 1) >= pages)
      throw Error("Page number is out of range");
    ...
    return {
      ...
      articles: saved.slice(page * NEWS_PAGE_SIZE, (page + 1) * NEWS_PAGE_SIZE)
    };
  });
}
